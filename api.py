...
from main import app, app_controller
...
def list_albums():
    ...
    return json.dumps([album.to_dict() for album in app_controller.albums])
...
def play_album(album_index: int):
    ...
    return json.dumps({"status": "success", "album": album.to_dict()})
...
def stop_playback():
    ...
    return json.dumps({"status": "success", "message": "Playback stopped."})
...
def pause_playback():
    ...
    return json.dumps({"status": "success", "message": "Playback paused."})
